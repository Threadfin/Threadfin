...
settingsCategory.push(new SettingsCategoryItem("{{.settings.category.general}}", "ThreadfinAutoUpdate,tuner,epgSource,epgCategories,epgCategoriesColors,api")); settingsCategory.push(new SettingsCategoryItem("{{.settings.category.files}}", "update,files.update,temp.path,cache.images,xepg.replace.missing.images,xepg.replace.channel.title,enableNonAscii"))
settingsCategory.push(new SettingsCategoryItem("{{.settings.category.streaming}}", "buffer,forceHttps,httpsPort,httpsThreadfinDomain,udpxy,buffer.size.kb,storeBufferInRAM,buffer.timeout,user.agent,ffmpeg.path,ffmpeg.options,vlc.path,vlc.options"))
