...
let progressHideTimeout: number | null = null; // Global timeout reference

function updateHeaderProgress(progress: any): void {
    const progressNav = document.getElementById("processing-status-nav") as HTMLElement;
    ...
    if (progress && progress.percentage === 0 && !progress.isProcessing) {
        ...
        const progressBar = progressNav.querySelector('.progress-bar') as HTMLElement;
        if (progressBar) {
    ...
    if (progress && progress.percentage > 0) {
        ...
        const spinner = progressNav.querySelector('.spinner-border') as HTMLElement;
        if (spinner) {
            if (progress.percentage < 100) {
...
function checkProcessingStatus(): void {
...
function checkForStuckMaintenance(): void {
