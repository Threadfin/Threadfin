...
func buildM3U(groups []string) (m3u string, err error) {
	...
	for _, channelNumber := range channelNumbers {
		...
		if channel.TvgLogo != "" {
			logo = imgc.Image.GetURL(channel.TvgLogo, Settings.HttpThreadfinDomain, Settings.Port, Settings.ForceHttpsForImages, Settings.HttpsPort, Settings.HttpsThreadfinDomain)
		}
