		...
		for i := len(Data.XMLTV.Files) - 1; i >= 0; i-- {
			...
			if err == nil {
				...
				for _, c := range xmltv.Channel {
					...
					channel["icon"] = imgc.Image.GetURL(c.Icon.Src, Settings.ForceHttps, Settings.HttpsPort, Settings.HttpsThreadfinDomain)
	...
	for _, dsa := range Data.Streams.Active {
		...
		case true:
			...
			if xepgChannel.XUpdateChannelIcon == true {
				var imgc = Data.Cache.Images
				xepgChannel.TvgLogo = imgc.Image.GetURL(m3uChannel.TvgLogo, Settings.ForceHttps, Settings.HttpsPort, Settings.HttpsThreadfinDomain)
			}
					...
					if channel, ok := value[mapping].(map[string]interface{}); ok {
						...
						if logo, ok := channel["icon"].(string); ok {
							...
							if xepgChannel.XUpdateChannelIcon && len(logo) > 0 {
								var imgc = Data.Cache.Images
								xepgChannel.TvgLogo = imgc.Image.GetURL(logo, Settings.ForceHttps, Settings.HttpsPort, Settings.HttpsThreadfinDomain)
							}
		...
		if err == nil {
			if xepgChannel.XActive && !xepgChannel.XHideChannel {
				if (Settings.XepgReplaceChannelTitle && xepgChannel.XMapping == "PPV") || xepgChannel.TvgName != "" {
					...
					channel.Icon = Icon{Src: imgc.Image.GetURL(xepgChannel.TvgLogo, Settings.ForceHttps, Settings.HttpsPort, Settings.HttpsThreadfinDomain)}
	...
	for d := 0; d < 4; d++ {
		...
		for t := dummyLength; t <= 1440; t = t + dummyLength {
			...
			if Settings.XepgReplaceMissingImages {
				poster.Src = imgc.Image.GetURL(xepgChannel.TvgLogo, Settings.ForceHttps, Settings.HttpsPort, Settings.HttpsThreadfinDomain)
...
func getPoster(program *Program, xmltvProgram *Program, xepgChannel XEPGChannelStruct, forceHttps bool) {
	...
	for _, poster := range xmltvProgram.Poster {
		poster.Src = imgc.Image.GetURL(poster.Src, Settings.ForceHttps, Settings.HttpsPort, Settings.HttpsThreadfinDomain)
